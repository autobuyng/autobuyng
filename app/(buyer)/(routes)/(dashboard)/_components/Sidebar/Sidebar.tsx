        ...
        {SIDEBAR_ITEMS.map((item) => {
          return (
            <Link
              key={item.id}
              href={item.path}
              className="flex items-center py-2  pl-5 xl:pl-6 gap-3 hover:bg-primary-100 w-full"
            >
              <Image src={item.Icon} alt={item.id} />
              <span className="capitalize font-medium text-[#4D4C4C] hover:text-primary-900">
                {item.text}
              </span>
            </Link>
          );
