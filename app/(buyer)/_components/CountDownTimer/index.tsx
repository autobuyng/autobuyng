  ...
  return (
    <div className="flex items-center justify-center mt-6 ">
      <div className="text-center p-6 ">
        <p className="text-sm text-gray-700">Kindly transfer exact amount  to the above account. Transaction is valid for: </p>
        {isExpired ? (
        ...
        <div className="flex items-center justify-center gap-6 mt-4">
          <button onClick={handleCancel} className="px-6 py-2 rounded-lg  bg-transparent border border-neutral-300 text-center">Cancel</button>
          <button onClick={() => router.push("/orders")} className="px-6 py-2 rounded-lg bg-primary-900 text-white "> Click here After transfer</button>
        </div>
      </div>
    </div>
  )
